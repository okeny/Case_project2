...
import (
	...
	mocks "project-cases/mocks/repository"
	"testing"
...
func TestCreate(t *testing.T) {
	...
	mockRepo := new(mocks.CaseRepository)

	Case := entity.Case{ID: 1, Title: "A", Description: "B"}
	//Expectations
	mockRepo.On("Save",&Case).Return(&Case, nil)
...
func TestFindByID(t *testing.T){
	...
	Case := entity.Case{ID: 1, Title: "A", Description: "B"}
	//Expectations
	mockRepo.On("FindById", 1).Return(&Case, nil)
	//creating testservice with a mock repo
	testService := NewCaseService(mockRepo)
...
func TestUpdate(t *testing.T){
	...
	Case := entity.Case{ID: 1, Title: "A", Description: "B"}
	//Expectations
	mockRepo.On("UpdateCase",&Case).Return(&Case, nil)
	//creating testservice with a mock repo
	testService := NewCaseService(mockRepo)
	result, _ := testService.Update(&Case)
	mockRepo.AssertExpectations(t)
